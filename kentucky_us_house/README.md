...
| 2002 | county-level | 311 | Kentucky State Board of Elections official report (user-supplied text export) |
| 2004 | county-level | 278 | Kentucky State Board of Elections official report (user-supplied text export) |
| 2006 | county-level | 276 | Kentucky State Board of Elections official report (user-supplied text export) |
| 2008 | county-level | 251 | Kentucky State Board of Elections official report (user-supplied text export) |
...
- 1992–2008: plain-text exports of the Kentucky State Board of Elections'
  ...
  `00Gen_Statewidebycounty.txt`, `2002statebycounty.txt`,
  `2004statebyCOUNTY.txt`, `STATEwidebycounty06.txt`,
  `STATEwide_by_candidate_by_county_gen_08.txt`), since this environment
  can't reach elect.ky.gov/sos.ky.gov directly. Every district's
  county-level vote counts were cross-checked against that district's own
  printed state total (all reconciled exactly), and further cross-validated
  against the MEDSL "U.S. House 1976–2022" dataset (which independently
  confirmed every candidate's statewide total for all nine years).
...
- The 2000, 2002, 2004, 2006, and 2008 source files did not print party
  labels directly (unlike 1992–1998, which had explicit `DEM`/`REP`/`REF`/etc.
  header rows) — party affiliations for those years' candidates were taken
  from the MEDSL 1976–2022 dataset, the same source used to independently
  confirm every year's statewide totals.
...
- 2004 CD5 (Harold Rogers) and 2008 CD3's write-in (Edward Martin) had no
  opposing candidate / recorded votes in the source; Rogers ran unopposed
  in 2004 CD5, and Edward Martin's row was blank (0 votes) in the 2008 CD3
  source table.
...
- Kentucky's congressional district boundaries shifted between decennial
  ...
  in CD5 from 1996 onward; Scott County appears in CD2's 2002 breakdown with
  a small vote count reflecting a split-county precinct) — this is reflected
  faithfully in the per-year district assignments, not an error.
